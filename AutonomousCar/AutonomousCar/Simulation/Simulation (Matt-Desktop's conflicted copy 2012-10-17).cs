...
namespace AutonomousCar.Simulation
{
    ...
    {
        private const string missionFile = @"c:\mtsuparking.json";
        private const string expFile = @"c:\parkingexp.txt";
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            if (cameraView == 2)
            {
                Vector2 headingVector = new Vector2((float)Math.Cos(car.Pose.Orientation), (float)Math.Sin(car.Pose.Orientation));
                v = Matrix.CreateLookAt(new Vector3(car.Pose.Position + -10f * headingVector, 8f), new Vector3(car.Pose.Position + 10f * headingVector, 0f), Vector3.UnitZ);
            }
