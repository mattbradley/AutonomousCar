...
namespace FarseerPhysics.DebugViews
{
    ...
    public class DebugViewXNA : DebugView, IDisposable
    {
        ...
        public void DrawBox(float hlength, float hwidth, Vector2 position, float orientation, Color color)
        {
            Vertices rectangleVertices = PolygonTools.CreateRectangle(hlength, hwidth, position, orientation);
            PolygonShape rectangleShape = new PolygonShape(rectangleVertices, 1f);
            DrawSolidPolygon(rectangleVertices.ToArray(), rectangleVertices.Count, color);
        }
        ...
        {
            DrawSolidPolygon(vertices, count, new Color(red, green, blue), true, true);
        }

        public void DrawSolidPolygon(Vector2[] vertices, int count, Color color)
        {
            // FIXME
            DrawSolidPolygon(vertices, count, color, true, true);
        }
        ...
        public void DrawSolidPolygon(Vector2[] vertices, int count, Color color, bool outline, bool alphaOutline)
        {
            if (!_primitiveBatch.IsReady())
            {
                throw new InvalidOperationException("BeginCustomDraw must be called before drawing anything.");
            ...
            Color colorFill = color * (outline && alphaOutline ? 0.5f : 1.0f);
        ...
        public void DrawArc(Vector2 center, float radius, float startAngle, float endAngle, float red, float green, float blue)
        {
            DrawArc(center, radius, startAngle, endAngle, new Color(red, green, blue));
        }

        public void DrawArc(Vector2 center, float radius, float startAngle, float endAngle, Color color)
        {
            if (!_primitiveBatch.IsReady())
            {
                throw new InvalidOperationException("BeginCustomDraw must be called before drawing anything.");
            }
            double increment = (endAngle - startAngle) / CircleSegments;
        ...
        {
            DrawSolidCircle(center, radius, Vector2.Zero, color);
        }

        public void DrawSolidCircle(Vector2 center, float radius, float red, float green, float blue)
        {
            DrawSolidCircle(center, radius, Vector2.Zero, new Color(red, green, blue));
        }
        ...
        {
            DrawSegment(start, end, color, 0.005f);
        }
        ...
        public void DrawSegment(Vector2 start, Vector2 end, Color color, float thickness)
        {
            ...
            dir.Normalize();
            Vector2 thickVec = dir * thickness;
            DrawSolidPolygon(
                new Vector2[] {
                    start + thickVec,
                    start - thickVec,
                    end - thickVec,
                    end + thickVec }, 4, color, true, false);
        }
        ...
        public void DrawPoint(Vector2 p, float size, Color color, bool outline)
        {
            Vector2[] verts = new Vector2[4];
            float hs = size / 2.0f;
